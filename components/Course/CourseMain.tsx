...
const CourseMain = () => {
    ...
    // const [oldSearchVal, setOldSearchVal] = useState(router.query.search);
    ...
    const handleCourseFilter = async (currentPage = 1, useTempFilter = false) => {
        ...
        fetch(`${process.env.APP_BACK_END_URL}/courses/search?${new URLSearchParams(params)}`, {
            method: "GET",
            headers: {
    ...
    const handleQuerySearch = () => {
        if(!searchVal) return;
        fetch(`${process.env.APP_BACK_END_URL}/courses/search?keyword=${searchVal}`, {
            method: "GET",
            headers: {
                Accept: "application/json",
                "Content-Type": "application/json",
    ...
    useEffect(() => {
        // setOldSearchVal(searchVal);
        let conditionArr = [
            { condition: 'cats?.length || selectedRating', handler: handleCourseFilter },
            { condition: 'searchVal', handler: handleQuerySearch },
            { condition: 'true', handler: getAllCourses }
        ];
        // if(oldSearchVal !== searchVal) { // searchVal changed
        //     [conditionArr[0], conditionArr[1]] = [conditionArr[1], conditionArr[0]];
